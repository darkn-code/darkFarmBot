...
def main():
    ...
    if not os.listdir(path):
        ...
        carpImag = os.listdir(path)
        for imagen in carpImag:
            file_metadata = {'name': imagen,
		             'parents':['11DS0bgW5lhY2ToW22hbvi9Vx5GEKxgBN']			
                            }
            media = MediaFileUpload('Desktop/darkFarmbot/imagenes/{}'.format(imagen), mimetype='image/jpg')
            file = service.files().create(body=file_metadata,
                                          media_body=media,
                                          fields='id').execute()
            print('File ID: {}'.format(file.get('id')))
        os.system('rm Desktop/darkFarmbot/imagenes/*.jpg')
